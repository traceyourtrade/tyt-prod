...
const CalendarPopup = () => {
  ...
  const StatCard = ({ icon, label, value, valueColor = "text-white", iconBg = "bg-white/5" }: { 
    ...
    <div className="group relative bg-[#1e1e1e] rounded-2xl p-4 border border-[#2a2a2a] hover:border-[#3a3a3a] transition-all duration-300">
    ...
    <div className={`fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${showTr ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      {dataToday.length === 0 ? (
        ...
        <div 
          ref={popupRef} 
          id="trade-details"
          className="w-full max-w-5xl max-h-[90vh] bg-[#141414] rounded-3xl flex flex-col border border-[#2a2a2a] shadow-2xl overflow-hidden"
        >
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a]">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={subtractOneDay}
                  className="w-7 h-7 rounded-lg bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
                >
                  <FontAwesomeIcon icon={faChevronLeft} className="text-gray-400 text-[10px]" />
                </button>
                <div className="px-3 py-1.5 bg-[#1e1e1e] rounded-lg">
                  <span className="text-gray-300 text-xs font-medium">{formatDate(dataDate)}</span>
                </div>
                <button
                  onClick={addOneDay}
                  className="w-7 h-7 rounded-lg bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
            ...
            <div className="flex items-center gap-3">
              <div className={`px-4 py-2 rounded-xl ${grossPnL >= 0 ? 'bg-emerald-500/15 border border-emerald-500/30' : 'bg-red-500/15 border border-red-500/30'}`}>
                <span className="text-gray-400 text-xs mr-2">Net P&L</span>
                <span className={`text-sm font-bold ${grossPnL >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {grossPnL >= 0 ? '+' : ''}{grossPnL.toFixed(2)}
                </span>
              </div>
              
              <div className="px-3 py-2 bg-[#1e1e1e] rounded-xl">
                <span className="text-gray-400 text-xs">{dataToday.length} trades</span>
              </div>

              <button
                onClick={handleShare}
                className="w-9 h-9 rounded-xl bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
              >
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#141414]">
            <div className="grid grid-cols-12 gap-5 mb-6">
              <div className="col-span-5 bg-[#1e1e1e] rounded-2xl p-4 border border-[#2a2a2a] h-48">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#2a2a2a]">
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Open Time</th>
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Symbol</th>
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Side</th>
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Net P&L</th>
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Net ROI</th>
                      <th className="px-5 py-4 text-left text-[10px] uppercase tracking-wider text-gray-500 font-semibold">R:R Ratio</th>
                      <th className="px-5 py-4 text-center text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {dataToday.map((data, index) => (
                      ...
                      >
                        <td className="px-5 py-4">
                          <span className="text-gray-300 text-sm font-medium">{data.OpenTime}</span>
                        </td>
                        <td className="px-5 py-4">
                          <span className="inline-flex items-center px-3 py-1.5 bg-emerald-500/15 text-emerald-400 rounded-lg text-xs font-semibold">
                            {data.Item}
                          </span>
                        </td>
                        <td className="px-5 py-4">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center justify-center gap-2">
                            <button 
                              ...
                              className="w-8 h-8 rounded-lg bg-[#252525] hover:bg-emerald-500/20 flex items-center justify-center transition-all duration-200"
                            >
                              <FontAwesomeIcon icon={faPenToSquare} className="text-gray-400 hover:text-emerald-400 text-xs" />
                            </button>
                            <button 
                              onClick={() => confirmDelete(data.id)}
                              className="w-8 h-8 rounded-lg bg-[#252525] hover:bg-red-500/20 flex items-center justify-center transition-all duration-200"
                            >
