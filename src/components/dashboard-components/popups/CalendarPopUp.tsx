...
const CalendarPopup = () => {
  ...
  const StatCard = ({ icon, label, value, valueColor = "text-white", iconBg = "bg-white/5" }: { 
    ...
    <div className="group relative bg-[#1e1e1e] rounded-xl p-3 border border-[#2a2a2a] hover:border-[#3a3a3a] transition-all duration-300">
      <div className="flex items-center gap-2">
        <div className={`w-7 h-7 ${iconBg} rounded-lg flex items-center justify-center flex-shrink-0`}>
          <FontAwesomeIcon icon={icon} className="text-white/70 text-xs" />
        </div>
  ...
  return (
    <div className={`fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${showTr ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
      {dataToday.length === 0 ? (
        <div 
          ref={popupRef} 
          className="w-full max-w-2xl bg-[#141414] rounded-3xl flex flex-col items-center justify-center p-8 border border-[#2a2a2a] shadow-2xl min-h-[400px] relative"
        >
          <button
            onClick={closePopup}
            className="absolute top-4 right-4 w-10 h-10 rounded-full bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
          >
          ...
          <div className="flex flex-row items-center gap-3 mb-4">
            <button
              ...
              className="w-8 h-8 rounded-full bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
            >
          ...
          <div className="w-20 h-20 rounded-full bg-[#1e1e1e] flex items-center justify-center mb-4">
            <Image src={Logo} alt="logo" width={50} height={50} className="opacity-70" />
          </div>
          ...
          <button
            ...
            className="group flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-400 text-white text-sm font-semibold rounded-xl transition-all duration-300"
          >
        ...
        <div 
          ref={popupRef} 
          id="trade-details"
          className="w-full max-w-5xl max-h-[90vh] bg-[#141414] rounded-2xl md:rounded-3xl flex flex-col border border-[#2a2a2a] shadow-2xl overflow-hidden"
        >
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 md:px-6 py-3 md:py-4 border-b border-[#2a2a2a] gap-3">
            <div className="flex items-center justify-between sm:justify-start gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-1.5 md:gap-2">
                <button
                  onClick={subtractOneDay}
                  className="w-6 h-6 md:w-7 md:h-7 rounded-lg bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
                >
                  <FontAwesomeIcon icon={faChevronLeft} className="text-gray-400 text-[10px]" />
                </button>
                <div className="px-2 md:px-3 py-1 md:py-1.5 bg-[#1e1e1e] rounded-lg">
              ...
              <button
                onClick={closePopup}
                className="w-7 h-7 rounded-lg bg-[#1e1e1e] hover:bg-red-500/20 flex items-center justify-center transition-colors duration-200 group sm:hidden"
              >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className={`px-2 md:px-4 py-1 md:py-2 rounded-lg md:rounded-xl ${grossPnL >= 0 ? 'bg-emerald-500/15 border border-emerald-500/30' : 'bg-red-500/15 border border-red-500/30'}`}>
                <span className={`text-[10px] md:text-sm font-bold ${grossPnL >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {grossPnL >= 0 ? '+' : ''}${Math.abs(grossPnL).toFixed(0)}
                </span>
              </div>
              
              <div className="px-2 md:px-3 py-1 md:py-2 bg-[#1e1e1e] rounded-lg md:rounded-xl">
                <span className="text-gray-400 text-[10px] md:text-xs">{dataToday.length}</span>
              </div>

              <button
                onClick={handleShare}
                className="w-6 h-6 md:w-9 md:h-9 rounded-lg md:rounded-xl bg-[#1e1e1e] hover:bg-[#2a2a2a] flex items-center justify-center transition-colors duration-200"
              >
              ...
              <button
                onClick={closePopup}
                className="w-6 h-6 md:w-9 md:h-9 rounded-lg md:rounded-xl bg-[#1e1e1e] hover:bg-red-500/20 flex items-center justify-center transition-colors duration-200 group hidden sm:flex"
              >
          ...
          <div className="flex-1 overflow-y-auto p-3 md:p-6 bg-[#141414]">
            <div className="flex flex-col md:grid md:grid-cols-12 gap-3 md:gap-5 mb-4 md:mb-6">
              <div className="w-full md:col-span-5 bg-[#1e1e1e] rounded-xl md:rounded-2xl p-3 md:p-4 border border-[#2a2a2a] h-36 md:h-48">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[9px] md:text-[10px] uppercase tracking-wider text-gray-500 font-medium">Cumulative P&L</span>
                  <div className={`px-1.5 md:px-2 py-0.5 rounded-md text-[9px] md:text-[10px] font-medium ${grossPnL >= 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
            ...
            <div className="bg-[#1e1e1e] rounded-xl md:rounded-2xl border border-[#2a2a2a] overflow-hidden">
                ...
                <table className="w-full min-w-[500px]">
                  ...
                  <tbody>
                    {dataToday.map((data, index) => (
                      <tr 
                        key={index} 
                        className="border-b border-[#2a2a2a] hover:bg-[#252525] transition-colors duration-150 group"
                      >
                        <td className="px-3 md:px-5 py-3 md:py-4">
                          <span className="text-gray-300 text-xs md:text-sm font-medium whitespace-nowrap">{data.OpenTime}</span>
                        </td>
                        <td className="px-3 md:px-5 py-3 md:py-4">
                          <span className="inline-flex items-center px-2 md:px-3 py-1 md:py-1.5 bg-emerald-500/15 text-emerald-400 rounded-lg text-[10px] md:text-xs font-semibold">
                            {data.Item}
                          </span>
                        </td>
                        <td className="px-3 md:px-5 py-3 md:py-4">
                          <span className={`inline-flex items-center px-2 md:px-2.5 py-1 rounded-lg text-[10px] md:text-xs font-semibold ${
                        ...
                        <td className="px-3 md:px-5 py-3 md:py-4">
                          <div className="flex items-center justify-center gap-1.5 md:gap-2">
                            <button 
                              ...
                              className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-[#252525] hover:bg-emerald-500/20 flex items-center justify-center transition-all duration-200"
                            >
                              <FontAwesomeIcon icon={faPenToSquare} className="text-gray-400 hover:text-emerald-400 text-[10px] md:text-xs" />
                            </button>
                            <button 
                              onClick={() => confirmDelete(data.id)}
                              className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-[#252525] hover:bg-red-500/20 flex items-center justify-center transition-all duration-200"
                            >
