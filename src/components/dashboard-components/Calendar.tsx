...
const Calendar = () => {
  ...
  const formatCurrencyDisplay = (num: number, preserveSign: boolean = false) => {
    const formatted = formatCompactCurrency(Math.abs(num), currency, exchangeRate, accountBalance, undefined, true);
  ...
  const getHeatmapColor = (profit: number, maxProfit: number, maxLoss: number) => {
    if (profit > 0) {
      const intensity = Math.min(profit / (maxProfit || 1), 1);
      if (intensity > 0.7) return "bg-emerald-500/30 dark:bg-emerald-500/25 border-emerald-500/50";
      if (intensity > 0.4) return "bg-emerald-500/20 dark:bg-emerald-500/15 border-emerald-500/30";
      return "bg-emerald-500/10 dark:bg-emerald-500/10 border-emerald-500/20";
    } else if (profit < 0) {
      const intensity = Math.min(Math.abs(profit) / (Math.abs(maxLoss) || 1), 1);
      if (intensity > 0.7) return "bg-red-500/30 dark:bg-red-500/25 border-red-500/50";
      if (intensity > 0.4) return "bg-red-500/20 dark:bg-red-500/15 border-red-500/30";
  ...
  const monthlyStats = useMemo(() => {
    ...
    for (let i = sortedDays.length - 1; i >= 0; i--) {
      ...
      if (streakType === null) {
        ...
        currentStreak = 1;
      } else if ((streakType === "win" && day.profit >= 0) || (streakType === "loss" && day.profit < 0)) {
        currentStreak += 1;
      } else {
        break;
      }
    ...
    return { 
      totalPnL, totalTrades, tradingDays, winRate, winningDays, losingDays,
      bestDay: bestDay.profit !== -Infinity ? bestDay : null,
      worstDay: worstDay.profit !== Infinity ? worstDay : null,
      maxProfit, maxLoss, currentStreak, streakType, avgDailyPnL
    };
      ...
      cells.push(
        ...
        >
          <span className={cn(
            ...
            isToday ? "text-primary font-bold" : hasTrades ? "text-foreground/80" : "text-muted-foreground/50"
          )}>
            {day}
          </span>
          ...
          {hasTrades && (
            <>
              <span className={cn(
                ...
                isProfit ? "text-emerald-600 dark:text-emerald-400" : isLoss ? "text-red-600 dark:text-red-400" : "text-muted-foreground"
              )}>
                {isProfit ? "+" : ""}{formatCurrencyDisplay(dayData.profit)}
              </span>
              
              <div className="flex items-center gap-0.5 mt-0.5">
                {Array.from({ length: Math.min(dayData.tradeLength, 5) }).map((_, i) => (
                  <div 
                    key={i} 
                    className={cn(
                      "w-1 h-1 rounded-full",
                      isProfit ? "bg-emerald-500" : isLoss ? "bg-red-500" : "bg-muted-foreground/50"
                    )} 
                  />
                ))}
                {dayData.tradeLength > 5 && (
                  <span className="text-[8px] text-muted-foreground/70 ml-0.5">+{dayData.tradeLength - 5}</span>
                )}
              </div>
            </>
          )}
                ...
                >
                  <div className="flex items-center justify-center mb-3">
                    ...
                    >
                      ...
                      <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", showYearView && "rotate-180")} />
                    </button>
                  </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="grid grid-cols-7 gap-1 sm:gap-1.5 mb-2">
              {["S", "M", "T", "W", "T", "F", "S"].map((day, i) => (
                <div key={i} className="text-center text-[10px] sm:text-xs font-semibold text-muted-foreground/60 uppercase tracking-wider py-1.5">
                  {day}
                </div>
              ))}
            </div>
          ...
          <div className="hidden sm:flex w-[68px] flex-col gap-1.5 pt-7">
            {weeklyProfits.map((profit, index) => (
              <motion.div
                ...
                whileHover={{ scale: 1.02 }}
              >
                <span className="text-[8px] text-muted-foreground/60 font-medium uppercase tracking-wide">W{index + 1}</span>
                <span className={cn(
                  "text-[11px] font-bold mt-0.5",
                  profit > 0 ? "text-emerald-600 dark:text-emerald-400" : profit < 0 ? "text-red-600 dark:text-red-400" : "text-muted-foreground/40"
                )}>
                  {profit !== 0 ? (profit > 0 ? "+" : "-") + formatCurrencyDisplay(profit) : "$0"}
                </span>
              </motion.div>
            ))}
          </div>
            ...
            {monthlyStats.bestDay && (
              ...
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium uppercase tracking-wide">Best Day</span>
                </div>
            ...
            {monthlyStats.worstDay && (
              ...
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[10px] text-red-600 dark:text-red-400 font-medium uppercase tracking-wide">Worst Day</span>
                </div>
