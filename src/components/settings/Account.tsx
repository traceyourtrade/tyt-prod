    ...
    <div className="bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#262626] rounded-xl p-4">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          {(account.broker === "MetaTrader 5" || account.broker === "MetaTrader 4") && (
            <div className="w-10 h-10 rounded-lg overflow-hidden bg-gray-100 dark:bg-[#1e1e1e] flex-shrink-0">
              <Image 
          ...
          <div className="min-w-0">
            <p className="text-gray-900 dark:text-white font-semibold truncate">{account.accountName}</p>
        ...
        >
          <button
            ...
            className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-[#1e1e1e] hover:bg-gray-200 dark:hover:bg-[#252525] flex items-center justify-center transition-colors"
          >
            <FontAwesomeIcon icon={faEllipsisVertical} className="text-gray-500 dark:text-gray-400 text-sm" />
          </button>
          
          {visibleOptions === index && (
            <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#262626] rounded-xl shadow-2xl z-50 overflow-hidden">
              <button 
                onClick={() => { setEditAccData(account); setEditAcc(); setVisibleOptions(null); }}
                className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors"
              >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-100 dark:bg-[#141414] rounded-lg p-3">
          ...
          <p className="text-sm text-gray-900 dark:text-white font-medium">{account.accountType}</p>
        </div>
        <div className="bg-gray-100 dark:bg-[#141414] rounded-lg p-3">
          <p className="text-xs text-gray-500 mb-1">Balance</p>
          <p className="text-sm text-gray-900 dark:text-white font-medium">{account.accountBalance}</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-200 dark:border-[#262626]">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-[#1e1e1e] hover:bg-gray-200 dark:hover:bg-[#252525] flex items-center justify-center transition-colors"
          >
            <FontAwesomeIcon icon={faPaperclip} className="text-gray-500 dark:text-gray-400 text-sm" />
          </button>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 lg:mb-8">
        <div>
          <h2 className="text-lg lg:text-xl font-bold text-gray-900 dark:text-white">Trading Accounts</h2>
        ...
        {accounts.length === 0 ? (
          <div className="bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#262626] rounded-2xl p-8">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-700 dark:text-gray-300 font-medium">No accounts added yet</p>
                ...
                accounts.map((account, index) => (
                  <tr key={index} className="border-b border-gray-200 dark:border-[#262626] hover:bg-gray-100 dark:hover:bg-[#1a1a1a] transition-colors">
                    <td className="px-6 py-4">
                      <div>
                        <p className="text-gray-900 dark:text-white font-semibold">{account.accountName}</p>
                        <p className="text-gray-500 text-xs mt-0.5">{account.broker}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      {(account.broker === "MetaTrader 5" || account.broker === "MetaTrader 4") && (
                        <div className="w-8 h-8 mx-auto rounded-lg overflow-hidden bg-gray-100 dark:bg-[#1e1e1e]">
                          <Image 
                            src={account.broker === "MetaTrader 5" ? MT5 : MT4} 
                            alt="Broker" 
                            className="w-full h-full object-contain" 
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="px-3 py-1 bg-gray-100 dark:bg-[#1e1e1e] text-gray-700 dark:text-gray-300 text-xs font-medium rounded-lg">
                        {account.accountType}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-gray-900 dark:text-white font-semibold">{account.accountBalance}</span>
                    </td>
                    <td className="px-6 py-4 text-center">
