...
export async function POST(req: NextRequest) {
    try {
        ...
        if (contentType.includes('multipart/form-data')) {
            ...
            if (apiName === 'uploadProfilePicture') {
                return await uploadProfilePictureHandler(formData, userId, token);
            }
        }
        ...
        switch (apiName) {
            case "uploadProfilePicture":
                ...
                return NextResponse.json({ error: "Upload profile picture must use form-data" }, { status: 400 });
