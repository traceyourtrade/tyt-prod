...
- **Backtesting Module:** Full-screen TradingView-powered chart for historical market simulation, with replay controls, trade placement, real-time P&L tracking, and session analytics. It includes persistent chart layouts, drawings, indicators, and a favorites toolbar saved per session. **Technical Note:** Timeframe changes use `setSymbol(symbol#tf_resolution_timestamp)` + `setResolution()` to ensure TradingView triggers fresh `getBars` callbacks for the custom replay datafeed. The dynamic timestamp suffix (e.g., `GBPUSD#tf_60_1735567890123`) is CRITICAL - stable suffixes cause TradingView to use cached data and skip getBars when switching back to previously-used timeframes. This causes a brief visual widget reload but is required for proper data filtering. `setVisibleRange()` maintains chart position centered on the replay timestamp after switches. **FX Replay Spec Compliance (Time-Driven Architecture):** `replayTimestampRef` is the ABSOLUTE source of truth for replay position - it NEVER changes during timeframe switches (only when user advances replay). All code paths use `setReplayTimeAndDeriveIndex()` as the canonical setter - replayTime is set first, index is always derived using the formula: a candle is "completed" when `(bar.time + intervalMs) <= replayTime`. This ensures the 12×5m→15m test case works correctly (12 candles on 5m = 60min elapsed → shows exactly 4 completed 15m candles). Scrolling is view-only. `callbacksReadyRef` gates playback controls and is set after the canonical setter completes.
