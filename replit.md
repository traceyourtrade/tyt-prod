...
- **Daily Journal:** Modern glassmorphic three-pane layout with fully responsive design. Features:
  - **Header:** Gradient accent bar (blue-purple-cyan), trade navigation (prev/next arrows, n/total counter), current trade info with symbol logo and P&L badge, Share and Save actions. Responsive with mobile toggle buttons for side panels.
  - **Left Panel:** Trade list sidebar with search, symbol logos, date, strategy tags, and color-coded P&L. Desktop: collapsible with toggle button. Mobile (< md): accessed via Layers icon opening slide-in drawer from left.
  - **Center Panel:** Chart/Notes/Running P&L tabs with TradingView chart integration using VPS API for Forex/Crypto/Indian markets. Timeframe selector (5y to D). Notes tab with Trade Note vs Daily Journal toggle, template selector, and quick tag pills.
  - **Right Panel:** Stats/Strategy/Executions/Attachments tabs. Stats shows Net P&L hero card, Side, Quantity, Pips, Return/Pip, Fees, Swap, Net ROI, Trade Rating (5-star). Strategy shows active strategy with rules compliance checklist. Executions shows entry/exit prices and duration. Attachments shows before/after screenshot gallery with upload. Desktop: collapsible with toggle button. Mobile (< lg): accessed via BarChart2 icon opening slide-in drawer from right.
  - **Mobile Drawers:** Framer Motion spring-animated slide-in panels with backdrop blur overlay and tap-to-dismiss. Trade list drawer (85% width, max 320px) from left. Stats drawer (90% width, max 360px) from right with tab navigation.
  - **Data Persistence:** Rules compliance preserved across saves even when strategy rules haven't loaded yet.
...
- **Backtesting Module:** Comprehensive backtesting system at `/backtesting/dashboard` with historical market simulation:
  ...
  - **Chart Persistence:** Auto-saves and restores TradingView drawings, indicators, favorite drawing tools, and chart state per session. Uses MongoDB to store chart layouts, study templates, and drawing templates. Auto-save triggers on drawing events, indicator changes, interval changes, and favorite tool changes. Saves before leaving and restores when returning to a session.
  - **Favorites Toolbar:** TradingView floating favorites bar for quick access to frequently-used drawing tools. Users can star tools in the drawing menu to add them to the floating toolbar. Favorites persist per session via MongoDB.
