...
- **Demo Mode:** Allows potential users to explore the platform without signing up. "Try Demo" button on login page creates a 1-hour demo session with `demoMode: true` JWT. Demo users see pre-populated sample data (15 trades, dashboard stats, demo account) via `src/lib/demo-data/`. Features are view-only - action buttons (Add Trade, etc.) show a "Demo Mode" upgrade modal when clicked. DemoModeBanner displays at the top with Sign Up CTA. Demo sessions bypass subscription checks via `/api/subscription/status` returning `hasAccess: true, status: 'demo', demoMode: true`.
