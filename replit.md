...
- **Subscription System:** Razorpay integration with **manual opt-in 3-day free trial** model. Pricing: ₹849/month or ₹8,199/year (20% off). 
  - **Pre-Trial Access Gating:** Before clicking "Start Free Trial", users can ONLY access the Dashboard. The sidebar hides all other navigation items (Daily Journal, Trades, Analysis, Tools, Backtesting). The `isPreTrial` flag in Layout identifies users with `canStartTrial: true` and `hasAccess: false`. Attempting to access restricted pages redirects to `/dashboard`.
