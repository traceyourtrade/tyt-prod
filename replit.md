...
- **Subscription System:** Razorpay integration with 5-day free trial for new users. Pricing: $9.49/month or $91.10/year (20% off). In-app checkout page at `/checkout` with monthly/yearly toggle, coupon code input, and Razorpay SDK modal for payments. Webhook handles subscription events (activated, charged, cancelled, halted). Sidebar displays "Go Pro" button with trial countdown or subscription status. Uses `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_PLAN_ID` (monthly), `RAZORPAY_PLAN_ID_YEARLY` (yearly), `RAZORPAY_WEBHOOK_SECRET` secrets. Pro-only features: Backtesting, Strategies, Playbook, AI Analysis (gated with SubscriptionGate component).
